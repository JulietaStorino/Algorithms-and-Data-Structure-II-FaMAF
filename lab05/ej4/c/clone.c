...
/* strlen(original) = 0;
En string_clone() se crea una variable clone que se almacena en la pila y se devuelve
como puntero. Sin embargo, cuando la función finaliza, la memoria en la pila se libera 
automáticamente, por lo que el puntero devuelto ya no es válido.
En el main sizeof() obtiene el tamaño del puntero y no la longitud real de la cadena.*/